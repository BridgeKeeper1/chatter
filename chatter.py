...
def on_send_message(data):
    ...
    # Anti-Spam Check
    text = (data.get("text") or "").strip()
    attachment = data.get("filename") if data.get("filename") and data.get("content") else None
    
    spam_check = check_anti_spam(username, text, attachment)
    if not spam_check["allowed"]:
        try:
            emit("system_message", spam_check["message"], room=f"user:{username}")
        except Exception:
            pass
        return
